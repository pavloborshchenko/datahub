...
VIEW_DATAHUB_PLATFORM = "metastore"
...
class LookerDashboardElement:
    ...
    def get_view_urns(self) -> typing.List[typing.Dict]:
        return [{"string": f"urn:li:dataset:(urn:li:dataPlatform:{VIEW_DATAHUB_PLATFORM},tableau.{v},PROD)" for v in self.looker_views}]
...
def get_actor_and_sys_time():
    actor, sys_time = "urn:li:corpuser:datahub", int(time.time()) * 1000
...
class ProperDataHubEvents:
    ...
    @staticmethod
    def make_dashboard_mce(looker_dashboard: LookerDashboard) -> DashboardDataHubEvents:
        actor, sys_time = get_actor_and_sys_time()
        ...
        #print(f"CREATED chart_mces OBJECTS: {chart_mces}")
        ...
        dashboard_mce = {
            ...
            "proposedSnapshot": {"com.linkedin.pegasus2avro.metadata.snapshot.DashboardSnapshot": {
                "urn": 	f"urn:li:dashboard:(looker,{looker_dashboard.get_urn_dashboard_id()})",
                "aspects": [
        ...
        return DashboardDataHubEvents(dashboard_mce=dashboard_mce, chart_mces=chart_mces)
...
class WorkaroundDatahubEvents:
    ...
    def make_dashboard_mce(looker_dashboard: LookerDashboard) -> DashboardDataHubEvents:
        ...
        upstreams = [{
            ...
            "dataset": chart_urn,
            "type":"TRANSFORMED"
        } for chart_urn in [mce["proposedSnapshot"]["com.linkedin.pegasus2avro.metadata.snapshot.DatasetSnapshot"]["urn"] for mce in chart_mces]]
...
def main():
    ...
    rest_emitter = DatahubRestEmitter("http://localhost:8080")
    ...
    file_path = pathlib.Path(LOOKER_MCE_FILE)
    ...
    workaround_wrote_something = False

    for looker_dashboard in looker_dashboards:
        looker_dashboard_events = ProperDataHubEvents.make_dashboard_mce(looker_dashboard)
        for mce in looker_dashboard_events.all_mces():
            #print(f"CREATED MCE OBJECT: {mce}")

            mce_class: MetadataChangeEvent = MetadataChangeEvent.from_obj(mce)
            rest_emitter.emit_mce(mce_class)

            if wrote_something:
                file.write(",\n")
            json.dump(mce, file, indent=4)
            wrote_something = True

        # Kafka way
        #dashboard_mce = looker_dashboard_events.dashboard_mce
        ##kafka_producer.produce(
        ##    topic=KAFKA_TOPIC,
        ##    key=dashboard_mce['proposedSnapshot']['com.linkedin.pegasus2avro.metadata.snapshot.DashboardSnapshot']['urn'],
        ##    value=dashboard_mce)
        ##kafka_producer.flush()
        #kafka_emitter.emit_mce_async(dashboard_mce, callback)
        #kafka_emitter.flush()

        #for mce in looker_dashboard_events.chart_mces:
        #    #kafka_producer.produce(
        #    #    topic=KAFKA_TOPIC,
        #    #    key=mce['proposedSnapshot']['com.linkedin.pegasus2avro.metadata.snapshot.ChartSnapshot']['urn'],
        #    #    value=mce)
        #    #kafka_producer.flush()
        #    kafka_emitter.emit_mce_async(mce, callback)
        #    kafka_emitter.flush()

        for mce in WorkaroundDatahubEvents.make_dashboard_mce(looker_dashboard).all_mces():
            #print(f"CREATED MCE <workaround> OBJECT: {mce}")

            #mce_class: MetadataChangeEvent = MetadataChangeEvent.from_obj(mce)
            #rest_emitter.emit_mce(mce_class)

            if workaround_wrote_something:
                workaround_file.write(",\n")
            json.dump(mce, workaround_file, indent=4)
            workaround_wrote_something = True
